...
class Individual(object):
    ...
    def init_gene_with_conv(self):
        ...
        # input_layer_num = 1（self.net_info.input_num が 1 かつ self.net_info.rows が 2 以上ならば）
        input_layer_num = int(self.net_info.input_num / self.net_info.rows) + 1
        # output_layer_num = 1 (self.net_info.out_num が 1 かつ self.net_info.rows が 2 以上ならば)
        output_layer_num = int(self.net_info.out_num / self.net_info.rows) + 1
        """
        layer_ids = [(self.net_info.cols - 3)]
        """
        layer_ids = [((self.net_info.cols - 1 - input_layer_num -
                      output_layer_num) + i) // (len(arch)) for i in range(len(arch))]
        prev_id = 0  # i.e. input layer
        current_layer = input_layer_num # = 1
        ...
        for i, idx in enumerate(layer_ids):
            ...
            current_layer += idx
            n = current_layer * self.net_info.rows + \
                np.random.randint(self.net_info.rows)
            block_ids.append(n)
            ...
            for j in range(1, self.net_info.max_in_num):
                self.gene[n][j + 1] = min_connect_id + \
        ...
        with open('child.txt', 'w') as fw_c:
            ...
            while self.num_gen < max_eval:
                ...
                for c in range(1 + self.lam):
                    writer_c.writerow(self._log_data_children(
                        net_info_type='full', start_time=start_time, pop=self.pop[c]))
                    writer_f.writerow(self._log_data_children(
                        net_info_type='active_only', start_time=start_time, pop=self.pop[c]))
                f.close()
                # replace the parent by the best individual
                if evaluations[best_arg] > self.pop[0].eval:
                    self.pop[0].copy(self.pop[best_arg + 1])
                else:
                    # modify the parent (neutral mutation)
                    self.pop[0].neutral_mutation(mutation_rate)

                # display and save log
                print(self._log_data(
                    net_info_type='active_only', start_time=start_time))
                fw = open(log_file, 'a')
                writer = csv.writer(fw, lineterminator='\n')
                writer.writerow(self._log_data(
                    net_info_type='full', start_time=start_time))
                fa = open('arch.txt', 'a')
                writer_a = csv.writer(fa, lineterminator='\n')
                writer_a.writerow(self._log_data(
                    net_info_type='active_only', start_time=start_time))
                fw.close()
                fa.close()
